export const stockTranslations = {
  "stock.input.searchAll": {
    en: "Search by product name, product code or barcode...",
    th: "ค้นหาด้วยชื่อสินค้า, รหัสสินค้า หรือบาร์โค้ด...",
  },
  "stock.title": {
    en: "Stock Update: Summary",
    th: "อัพเดทสต๊อก: สรุป",
  },
  "stock.details.title": {
    en: "Stock Update: Product Details",
    th: "อัพเดทสต๊อก: รายละเอียดสินค้า",
  },
  "stock.details.subtitle": {
    en: "View Product Information",
    th: "ดูข้อมูลสินค้า",
  },
  "stock.title.lot": {
    en: "Stock Update: Detail by Lot",
    th: "อัพเดทสต๊อก: รายละเอียดตามล็อต",
  },
  "stock.title.lotBatch": {
    en: "Stock Update: Detail by Lot Batch",
    th: "อัพเดทสต๊อก: รายละเอียดตามล็อต Batch",
  },
  "stock.table.no": { en: "No.", th: "ลำดับ" },
  "stock.table.image": { en: "Image", th: "รูปภาพ" },
  "stock.table.itemId": { en: "Item ID", th: "รหัสสินค้า" },
  "stock.table.itemName": { en: "Item Name", th: "ชื่อสินค้า" },
  "stock.table.productId": { en: "Product Code", th: "รหัสสินค้า" },
  "stock.table.productName": { en: "Product Name", th: "ชื่อสินค้า" },
  "stock.table.barcode": { en: "Barcode", th: "บาร์โค้ด" },
  "stock.table.group": { en: "Group", th: "กลุ่ม" },
  "stock.table.subGroup": { en: "Sub Group", th: "กลุ่มย่อย" },
  "stock.table.tag": { en: "Tag", th: "แท็ก" },
  "stock.table.non-tag": { en: "Non-Tag", th: "ไม่มีแท็ก" },
  "stock.table.lot": { en: "Lot", th: "ล็อต" },
  "stock.table.lotBatch": { en: "Lot Batch", th: "ล็อต Batch" },
  "stock.table.totalLot": { en: "Total Lot", th: "จำนวนล็อตรวม" },
  "stock.table.totalQty": { en: "Total Qty", th: "จำนวนรวม" },
  "stock.table.uom": { en: "UoM", th: "หน่วยนับ" },
  "stock.table.category": { en: "Categories", th: "หมวดหมู่" },
  "stock.table.totalLocation": { en: "Total Location", th: "จำนวนตำแหน่งรวม" },
  "stock.table.warehouse": { en: "Warehouse", th: "คลังสินค้า" },
  "stock.table.zone": { en: "Zone", th: "โซน" },
  "stock.table.area": { en: "Area", th: "พื้นที่" },
  "stock.table.subArea": { en: "Sub Area", th: "พื้นที่ย่อย" },
  "stock.table.shelfLife": { en: "Shelf Life (Days)", th: "อายุสินค้า (วัน)" },
  "stock.table.expiredDate": { en: "Expired Date", th: "วันหมดอายุ" },
  // Details --------
  "stock.details.productInfo": {
    en: "Product Information",
    th: "ข้อมูลสินค้า",
  },
  "stock.details.productDetails": {
    en: "Product Details",
    th: "รายละเอียดสินค้า",
  },
  "stock.details.lotInfo": { en: "Lot Information", th: "ข้อมูลล็อต" },
  "stock.details.inventoryInfo": {
    en: "Inventory Information",
    th: "ข้อมูลสต็อก",
  },
  "stock.details.location": { en: "Location", th: "ตำแหน่ง" },
  "stock.details.productGroup": { en: "Product Group", th: "กลุ่มสินค้า" },
  "stock.details.productList": { en: "Product List", th: "รายการสินค้า" },
  "stock.details.expirationInfo": {
    en: "Notification of Expiration Date",
    th: "แจ้งเตือนวันหมดอายุ",
  },
  "stock.details.productSpecs": {
    en: "Product Specs",
    th: "ข้อมูลจำเพาะสินค้า",
  },
  "stock.details.productTypes": { en: "Product Types", th: "ประเภทสินค้า" },
  "stock.details.expirationNotice": {
    en: "Expiration Notice",
    th: "การแจ้งเตือนวันหมดอายุ",
  },
  "stock.details.batch": {
    en: "Batch",
    th: "แบทช์",
  },
  "stock.details.status": {
    en: "Status",
    th: "สถานะ",
  },
  "stock.details.serialNumber": {
    en: "Serial Number",
    th: "หมายเลขซีเรียล",
  },
  "stock.details.weight": {
    en: "Weight",
    th: "น้ำหนัก",
  },
  "stock.details.length": {
    en: "Length",
    th: "ความยาว",
  },
  "stock.details.height": {
    en: "Height",
    th: "ความสูง",
  },
  "stock.details.width": {
    en: "Width",
    th: "ความกว้าง",
  },
  "stock.details.productCode": {
    en: "Product Code",
    th: "รหัสสินค้า",
  },
  "stock.details.productName": {
    en: "Product Name",
    th: "ชื่อสินค้า",
  },
  "stock.details.productBarcode": {
    en: "Barcode",
    th: "บาร์โค้ด",
  },
  "stock.details.lotNumber": {
    en: "Lot Number",
    th: "หมายเลขล็อต",
  },
  "stock.details.size": {
    en: "Size",
    th: "ขนาด",
  },
  "stock.details.color": {
    en: "Color",
    th: "สี",
  },
  "stock.details.model": {
    en: "Model",
    th: "รุ่น",
  },
  "stock.details.brand": {
    en: "Brand",
    th: "แบรนด์",
  },
  "stock.details.total": {
    en: "Total",
    th: "รวม",
  },
} as const;
