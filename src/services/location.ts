...
export const fetchZones = async (): Promise<Zone[]> => {
  const response = await apiClient.get('/Locations/zone/all');
  return response.data;
};

export const fetchAreas = async (zoneCode: string): Promise<Area[]> => {
  if (!zoneCode || zoneCode === 'All Zones') return [];
  const response = await apiClient.get(`/Locations/area/all?zoneCode=${zoneCode}`);
  return response.data;
};

export const fetchSubAreas = async (zoneCode: string, areaCode: string): Promise<SubArea[]> => {
